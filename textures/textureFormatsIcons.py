...
def recognizeIconTextureFormat(texPathList, texFormatList):
...
def getIconsTextureFormat(settings, fullFileName):
    ...
    if len(texPathList) == 0:
        ...
        questions.printError("No textures were found in the model. Please try again.", False)
        ...
        (png8Counter, dxt1Counter, plainPngCounter, texFolder, texFile) = recognizeIconTextureFormat(texPathList, texFormatList)
        # Get the file name
        ...
        if plainPngCounter == 1:
            # This is a plain png texture, meaning that this is for MUA1 or MUA2 (not PS2 or PSP)
            ...
            if settings["pcOnly"] == False:
                ...
                textureFolderList = ["MUA1 PC, Steam, PS3, and 360", "MUA1 Wii and Xbox", "MUA2 Wii"]
            else:
                ...
                textureFolderList = ["MUA1 PC and Steam"]
            # Determine the format that's in use.
            textureFormat = findFolder(texFolder, textureFolderList)
        elif png8Counter == 1:
            # This is an indexed texture, either regular PNG8 or indexed-alpha
            # Determine if this is for PC or consoles, which will determine which texture list to use.
            if settings["pcOnly"] == False:
                # The model is being processed for all consoles.
                # Set up the texture folder list with png8 texture options that are compatible with all consoles.
                textureFolderList = ["XML1 All", "XML2 Xbox", "XML2 PC and Xbox", "XML2 All", "MUA1 PS2 and PSP", "MUA2 PS2 and PSP"]
            else:
                # The model is being process for PC only.
                # Set up the texture folder list with png8 texture options that are compatible with PC only.
                textureFolderList = ["XML2 PC"]
            # Determine the format that's in use.
            textureFormat = findFolder(texFolder, textureFolderList)
        elif dxt1Counter == 1:
            # This is a dxt1 texture, which is only used with oversized XML2 PC textures.
            # Determine if this is for PC or consoles, which will determine which texture list to use.
            if settings["pcOnly"] == False:
                # The model is being processed for all consoles.
        ...
        questions.printError("Two or more textures were found in the model. Power icons should only have one texture applied. Please try again.", False)
    # Return the collected value
    return textureFormat, fileName