...
def GetTextureType(textures_list, settings_dict, asset_type):
    ...
    if len(textures_list) == 0:
        ...
        if asset_type in ['Skin', 'Mannequin', '3D Head', 'Other']:
            ...
            if settings_dict['untextured_okay'] == True:
                ...
                # Give an error.
                questions.PrintError('This model has no textures. The settings have been configured to not allow models without textures.', system_exit = True)
        else:
            # This is an asset that can't be without textures.
            # Give an error.
            questions.PrintError(f'This model has no textures. Models of asset type {asset_type} must always have textures.', system_exit = True)
    else:
        ...
        if len(textures_list) > 1:
            ...
            if not(asset_type in ['Skin', 'Mannequin', '3D Head', 'Other']):
                ...
                # Give an error.
                questions.PrintError(f'This model has more than 1 texture. Models of type {asset_type} must always have 1 texture.', system_exit = True)
...
def GetTextureInfo(application_path, input_file_path, settings_dict, asset_type):
    ...
    texture_type = GetTextureType(textures_list, settings_dict, asset_type)
